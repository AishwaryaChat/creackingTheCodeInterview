...
RandomizedSet.prototype.remove = function (val) {
  if (this.map.has(val)) {
    ...
    let valAtLastIndex = this.indexMap.get(this.count);
    this.map.set(valAtLastIndex, idx);
    this.indexMap.set(idx, valAtLastIndex);
...
RandomizedSet.prototype.getRandom = function () {
  let randomNumber = Math.floor(Math.random() * (this.count + 1));
  return this.indexMap.get(randomNumber);
};
