...
function solve(A, B, C) {
  ...
  for (let j = 0; j < C.length; j++) {
    ...
    for (let i = 0; i < prefix.length; i++) {
      const charCode = prefix[i].charCodeAt(0) - 97;
      curr = curr.next[charCode];
    }
    const sorted = curr.prefixWords
...
const A = ["abcd", "aecd", "abaa", "abef", "acdcc", "acbcc"];
const B = [2, 1, 3, 4, 6, 5];
const C = ["ab", "abc", "a"];

// const A = ["aaaa", "aacd", "abaa", "abaa", "aadcc"];
// const B = [3, 4, 1, 2, 6 ]
// console.log(solve(A, B, "ab").next[0].next[1].next[0].next[0]);
