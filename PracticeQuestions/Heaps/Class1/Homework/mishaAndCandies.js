...
function solve(A, B) {
  ...
  for (let i = 0; i < A.length; i++) {
    const candies = minHeap.pop();
    if (candies > B) continue;
    const candiesToEat = Math.floor(candies / 2);
    ans += candiesToEat;
    if (minHeap.getSize() > 0) {
      const nextMin = minHeap.pop() + (candies - candiesToEat);
      minHeap.push(nextMin);
    }
  }
  return ans;
}
